...
//            localStringBuilder.append("\n");
            ...
            localStringBuilder.append(" SSID:").append(mWifiList.get(i).SSID)
                    .append(" BSSID：").append(mWifiList.get(i).BSSID)
                    .append(" \n接入性能：").append(mWifiList.get(i).capabilities)
                    .append(" \n频率：").append(mWifiList.get(i).frequency)
                    .append("\nRSSI：").append(mWifiList.get(i).level + 96)
                    .append(" \n连接描述：").append(mWifiList.get(i).describeContents()).append("\n\n");
        }
...
//            imageView.setImageDrawable(getResources().getDrawable(R.drawable.ic_wifi_s3));
    ...
    private void conneted(){
        ...
        new StaticIpSet(context, ipAddress, preLength, getWay, dns1).confingStaticIp(wifiConfig);
    ...
    private void unconneted(){
        ...
        //连接指定wifiConfig
        mWifiManager.enableNetwork(mWifiManager.addNetwork(wifiConfig), true);
    }
}